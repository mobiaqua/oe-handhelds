...
#MobiAqua: added fix-ld-macosx.patch
...
do_install() {
	...
	#MobiAqua: fix up paths for stage
	sed -i -e "s,^realpath=.*,realpath=${STAGING_BINDIR}/perl," ${D}${bindir}/perl
	sed -i -e "s,^realpath=.*,realpath=${STAGING_BINDIR}/perl${PV}," ${D}${bindir}/perl${PV}
}
